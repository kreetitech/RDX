...
Table.prototype.createSync = function(values) {
	...
	var sql = "insert into " + me.tableName + " (" + me.fields.map(function(f){return f.toSql();}).join(",") + ") values (" + 
	    me.fields.map(function(f) { return (value[f.name] === undefined ? 'null' : "'" + value[f.name] + "'"); }).join(",") + ")";
	console.log(sql);
